...
class Qontroller(object):
	...
	Super class which handles serial communication, device identification, and logging.
		...
		error_desc_dict = Q8_ERRORS			Error code descriptions
	...
	The log handler may be used to catch and handle dynamically certain errors, as they arise. In the following example, it is set up to raise a RuntimeError upon reception of errors E01, E02, and E03:
	...
	def __init__(self, *args, **kwargs):
		...
		self.error_desc_dict = Q8_ERRORS				# Error code descriptions
		...
		# Open serial port directly, get device id
			...
			for t in range(10):
				...
				# Allow time for subsequent bytes
			...
			if ob is not None:
				...
				print('Qontroller.__init__: Warning: Unable to determine ID of device on port {0} (received response {1}, "{2}").'.format(self.serial_port_name, ":".join("{:02x}".format(ord(c)) for c in response), response.replace('\n', '\\n')))

			

		else:

			raise AttributeError('At least one of serial_port_name or device_id must be specified on Qontroller initialisation.')
	...
	def log_append (self, type='err', id='', ch=0, value=0, desc='', raw=''):
		...
		self.log.append({'timestamp':time.asctime(), 'proctime':time.time()-self.init_time, 'type':type, 'id':id, 'ch':ch, 'value':value, 'desc':desc, 'raw':raw})
	...
	def issue_command (self, command_id, ch=None, operator='', value=None, n_lines_requested=2**31, target_errors=None, output_regex='(.*)', special_timeout = None):
		...
		while (True):
				...
				if (RESPONSE_OK in lines):
					...
					# print ('breaking from OK (command was {:})'.format(command_id))

					break

				elif (len(lines) >= n_lines_requested):

# 					print ('breaking from NLINES ({:} lines received; command was {:})'.format(len(lines), command_id))

					break

				elif not all([err['id'] not in target_errors for err in errs]):

					# print ('breaking from ERR (command was {:})'.format(command_id))
	...
	# 					print ('breaking from TIMEOUT (command was {:})'.format(command_id))
...
class QXOutput(Qontroller):
	...
	def __init__(self, *args, **kwargs):
		...
		self.p = None			# Channel powers (direct access)
	...
	def __setattr__(self, attr, val):
		...
		FIXME: override to prevent overwriting v i p

		"""

		super(type(self), self).__setattr__(attr, val)
